...
namespace SBC
{
	...
	public class SteelBattalionController {
		...
		public void Init(int Interval) {
			...
			pollTimer.Start();//disabling pollTimer allows starting and restarting device.  NEED to figure this out
             */
            reader.DataReceivedEnabled = true;
            //reader.DataReceived -= (OnRxEndPointData);
            reader.DataReceived += mEp_DataReceived;

			TestLEDs();
            if (updateGearLights)
                GearLightsRefresh((int)unchecked((sbyte)rawControlData[25]));
			RefreshLEDState();
		...
		public void UnInit() {
            ...
            //something is happening with pollTimer when uninitializing
			pollTimer.Stop();
